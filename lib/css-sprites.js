...
module.exports = function (opt) {
  ...
  function queue (file, img) {
    ...
    var spriteName = replaceExtension(file.relative, '')
    if(!opt.filesName){
      spriteName = spriteName.replace(/\/|\\|\ /g, '-');
    }else{
      var match = spriteName.split(/\/|\\|\ /);
      var len = match.length;
      if(len>1){
        var name = match[len-1].replace(/@2x$/g,'');
        if(!isNaN(name)){
          iconArea = Number(name);
        }
        spriteName = match[len-2];
        //console.log(spriteName);
      }
    }
    
    layer.addItem({
      height: (iconArea || img.height()) + 2 * opt.margin,
      width: (iconArea || img.width()) + 2 * opt.margin,
      meta: {
        name: spriteName,
        img: img,
